...
def procesar_archivo1():
    # Leer CSV
    status_df = pd.read_csv('Apps-Script/status_history.csv')
    ...
    latest_status = status_df.sort_values('status_date').groupby('asset_id').tail(1)

    today = pd.Timestamp(datetime.now())

    # Calcular días
    latest_status['dias_en_estado'] = (today - latest_status['status_date']).dt.days
